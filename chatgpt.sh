...
list_models() {
	models_response=$(curl "$OPENAI_API_URI/v1/models" \
		-sS \
...
request_to_completions() {
	...
	curl "$OPENAI_API_URI/v1/completions" \
		-sS \
...
request_to_image() {
	local prompt="$1"
	image_response=$(curl "$OPENAI_API_URI/v1/images/generations" \
		-sS \
		-H 'Content-Type: application/json' \
		-H "Authorization: Bearer $OPENAI_API_KEY" \
		-d '{
...
request_to_chat() {
	...
	curl "$OPENAI_API_URI/v1/chat/completions" \
		-sS \
		-H 'Content-Type: application/json' \
		-H "Authorization: Bearer $OPENAI_API_KEY" \
		-d '{
